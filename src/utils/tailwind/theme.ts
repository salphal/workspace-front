 ...
 */
const getThemeName = (name: ThemeName) => `theme-${name}`;

/**
 * 根据名称切换主题
 * @param {ThemeName} name - 主题名称
 */
export function toggleTheme(name: ThemeName) {
  ...
  root.classList.forEach((cls) => {
    if (themeClassRegex.test(cls)) {
      root.classList.replace(cls, themeName);
    }
  });
  // 如果没有找到任何主题类，直接添加新的主题类
  if (!root.classList.contains(themeName)) {
